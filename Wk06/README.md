...
- **Task**: Take a sample of the **JSON outputs** generated by all teams for Assignment 02 and use Vibe Coding to generate an app that loads them into a **Google Sheet**.
- **Procedure**:
  1. **Collect Samples**: For each AO2 team, collect at least two JSON files (named _**TEAMNAME-TEST-01.json**_ to _**TEAMNAME-TEST-05.json**_). You will also need a copy of the _**WHOLE-CLASS-TEST-FILES.csv**_ metadata file.
  2. **Vibe Code the Converter App**: Use Vibe Coding to generate a dedicated, single-file HTML app. The app must request that the user upload a CSV file containing metadata about trial extractions. (This is the _**WHOLE-CLASS-TEST-FILES.csv**_ metadata file). It must also request that the user upload a batch of JSON files that reflect those extractions. When the button is pressed, the app will aggregate the information contained in the JSON files and the metadata file and display the data formatted as **CSV (comma-separated values)** in a large **output area**.
  3. **Run the App**: When requested, upload the CSV metadata file and the batch of JSON files you are pooling. Click the button, copy the resulting CSV text from the output area, and paste it into a Google Sheet.

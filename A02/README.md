...
- **Procedure**:
  1. The team must agree on the **single, best Master Prompt** and execute it in AI Studio to generate the final `.html` file.
...
- _**TEAMNAME-EXTRACTOR.html**_
  - The Vibe-Coded Entity Extractor App. This **must be the single, self-contained HTML file** that contains all the necessary JavaScript logic within the `<script>` tags.
  - **Rename this single file** to _**TEAMNAME-EXTRACTOR.html**_.
  - **(Crucial Note)**: Do not submit any extraneous files generated by the AI Studio (e.g., `.tsx`, `index.js`, or `services/`). The `.html` file must run independently.
